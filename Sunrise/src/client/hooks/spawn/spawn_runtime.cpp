...
[[nodiscard]] bool is_controlled_object(const void* object) noexcept {
    ...
    const std::uint32_t controlled = teleport::local_player_handle();
    if (controlled == kInvalidDatum) {
        return false;
    }

    constexpr std::size_t kSummonObjectHandleOffset = 0x2C;
    std::uint32_t candidate = kInvalidDatum;
    return safe_read(static_cast<const std::byte*>(object) + kSummonObjectHandleOffset, candidate)
           && (controlled & 0x1FFFU) == (candidate & 0x1FFFU);
}
...
void service_request() noexcept {
    ...
    teleport::CameraPose cameraPose{};
    if (!teleport::camera_pose(cameraPose)) {
        return;
    }

    const teleport::Vector& camera = cameraPose.position;
    const teleport::Vector& forward = cameraPose.forward;
    ...
    bool placed = false;
    if (g_request.origin == Origin::player) {
        void* const component = teleport::local_player_component();
        placed = component != nullptr && teleport::read_position(component, position);
    } else {
...
void __fastcall summon_update(void* object, void* input, void* authored) noexcept {
    ...
    if (is_controlled_object(object)) {
        poll_shortcuts();
        service_activations();
        service_request();
    }
...
bool install() noexcept {
    ...
    if (g_resolver == nullptr || g_gameModule == nullptr
        || !hooking::detour::install({update, reinterpret_cast<void*>(&summon_update)},
                                     g_updateHook)) {
        uninstall();
        core::log::write(core::log::Channel::client,
                         core::log::Level::warn,
