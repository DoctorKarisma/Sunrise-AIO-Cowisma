...
namespace socket_entries = state::build_data::socket_entry_lists;

namespace ability_catalog = state::build_data::abilities;
...
[[nodiscard]] WeaponRow* selected_armor() noexcept {
    ...
    const std::size_t selected =
        (std::min)(g_selectedArmorBySlot[g_activeArmorSlot], rows.size() - 1);
...
[[nodiscard]] bool
armor_name_matches_character_class(std::string_view name,
    ...
    case state::CharacterClass::titan:
        return ends_with_any(
            {" Helm", " Helmet", " Gauntlets", " Plate", " Cuirass", " Greaves", " Mark"});
    case state::CharacterClass::hunter:
        return ends_with_any(
            {" Mask", " Cowl", " Grips", " Grasps", " Vest", " Harness", " Strides", " Cloak"});
    case state::CharacterClass::warlock:
        return ends_with_any(
            {" Hood", " Gloves", " Wraps", " Robes", " Raiment", " Boots", " Bond"});
    }

    return false;
}

[[nodiscard]] bool armor_matches_character_class(const item_details::Definition& detail,
                                                 state::CharacterClass characterClass) noexcept {
    ...
    const bool hasAnyClassSpecificArt = std::any_of(
        detail.artArrangementIndices.begin() + 1,
        detail.artArrangementIndices.end(),
        [](std::uint16_t index) noexcept { return index != item_details::kUnavailableArtIndex; });
...
[[nodiscard]] bool is_exotic_armor_detail(const item_details::Definition& detail) noexcept {
    ...
    return is_armor_detail(detail) && weapon_rarity(detail.definitionHash) == WeaponRarity::exotic;
}
...
[[nodiscard]] bool has_renderable_gear_art(const item_details::Definition& detail) noexcept {
    ...
    return std::any_of(
        detail.artArrangementIndices.begin(),
        detail.artArrangementIndices.end(),
        [](std::uint16_t index) noexcept { return index != item_details::kUnavailableArtIndex; });
}
...
[[nodiscard]] PlugCategoryMask category_for_socket(const item_details::Definition& detail,
    ...
    case SocketKind::shader:
        return kPlugCategoryShader;

    case SocketKind::ornament:
        return kPlugCategoryOrnament;

    case SocketKind::mod:
        return weapon ? kPlugCategoryMod : kPlugCategoryArmorMod;

    case SocketKind::catalyst:
        return kPlugCategoryCatalyst;

    case SocketKind::masterwork:
        return kPlugCategoryMasterwork;

    case SocketKind::tracker:
        return kPlugCategoryTracker;
...
void rebuild_plug_categories() {
    ...
    for (std::size_t index = 0; index < copiedRules; ++index) {
        ...
        const bool armorIntrinsicLane = is_armor_detail(detail)
                                        ...
                                        && detail.socketTypes[rule.lane] == 677;
    ...
    for (std::size_t definitionIndex = 0; definitionIndex < g_plugCategoryByDefinition.size();
         ++definitionIndex) {
        ...
        const bool publishedSocketPlug =
            definitionIndex <= 0xFFFFU
            && state::build_data::is_socket_plug_valid(static_cast<std::uint16_t>(definitionIndex));
...
void rebuild_plugs(const WeaponRow& weapon) {
    ...
    const std::uint16_t pendingDefinition =
        hasPending ? g_pendingPlugDefinitions[g_selectedLane] : 0;
...
void apply_pending(const WeaponRow& weapon) noexcept {
    ...
    if (failed != 0) {
        (void)std::snprintf(g_message.data(),
                            ...
                            failed);
        core::ui::toast::post(
            core::ui::toast::Type::warning, "Gear Editor", "Applied with some failures.");
    } else if (persisted) {
        (void)std::snprintf(g_message.data(),
                            g_message.size(),
                            ...
                            applied);
        core::ui::toast::post(
            core::ui::toast::Type::success, "Gear Editor", "Perk changes applied and saved!");
...
void draw_weapon_details(WeaponRow& weapon) noexcept {
    ...
    ImGui::TextDisabled("Instance  0x%llX",
                        static_cast<unsigned long long>(weapon.item.instanceSoid));
    ...
    const std::size_t perkColumns =
        (std::max)(std::size_t{1}, (std::min)(perkCount, std::size_t{6}));
...
void draw_inventory_panel() noexcept {
    ...
    const float horizontal =
        (availableWidth - scaled(kInventoryGap) * static_cast<float>(kWeaponColumns - 1))
...
[[nodiscard]] bool draw_plug_card(const PlugChoice& choice,
    ...
    tooltip_name_hash(
        choice.name, choice.definition.definitionHash, choice.definition.definitionIndex);
...
void draw_plug_filter_popup() noexcept {
    ...
    const float resetX =
        (std::max)(ImGui::GetCursorPosX(), ImGui::GetContentRegionMax().x - scaled(46.0F));
...
void draw_unrestricted_warning() noexcept {
    ...
    if (ImGui::IsItemHovered()) {
        ...
        ImGui::TextUnformatted(
            "Incompatible plugs may behave incorrectly or make the item unusable.");
...
void draw_plug_browser(WeaponRow& weapon) noexcept {
    ...
    const float gaps =
        ImGui::GetStyle().ItemSpacing.x * (g_filter == PlugFilter::all ? 3.0F : 2.0F);
...
void draw_weapon_browser(WeaponRow& weapon) noexcept {
    ...
    if (!armorCatalog) {
        ...
        if (activeFilters == 0) {
            ...
            (void)std::snprintf(
                filterLabel.data(), filterLabel.size(), "Filters (%zu)", activeFilters);
        }
    ...
    const std::size_t rowCount =
        (g_visibleWeaponCatalog.size() + static_cast<std::size_t>(columns) - 1)
...
randomizer_replacement_pool(const RandomizerItem& item,
    ...
    for (std::size_t index = 0; index < definitionCount && index <= 0xFFFFU; ++index) {
        ...
        if ((item.weapon && !is_weapon_detail(detail)) || (!item.weapon && !is_armor_detail(detail))
...
[[nodiscard]] bool randomizer_perk_lane(const item_details::Definition& detail,
    ...
    return (categories
            & (kPlugCategoryWeaponPerk | kPlugCategoryIntrinsic
...
randomizer_inventory_count_for_slot(const state::CharacterState& character,
...
void fill_randomizer_slot(const state::CharacterState& character,
    ...
    const RandomizerReplacementPool& pool = randomizer_replacement_pool(
        slotTemplate, replacementPools, slotTemplate.weapon ? std::nullopt : armorClass);
...
void fill_randomizer_inventory(const state::CharacterState& character,
    ...
    if (randomizer_wants_weapons(targetMode)) {
        ...
        for (const inventory::EquipmentSlot slot : kWeaponSlots) {
            ...
            fill_randomizer_slot(character, slot, armorClass, rng, replacementPools, result);
        }
    }
...
randomizer_armor_class(RandomizerArmorClass selection) noexcept {
    ...
    case RandomizerArmorClass::titan:
        return state::CharacterClass::titan;

    case RandomizerArmorClass::hunter:
        return state::CharacterClass::hunter;

    case RandomizerArmorClass::warlock:
        return state::CharacterClass::warlock;
...
run_randomizer(const state::CharacterState& character,
    ...
    const std::optional<state::CharacterClass> armorClass = randomizer_armor_class(armorClassMode);

    fill_randomizer_inventory(character, targetMode, armorClass, rng, replacementPools, result);
    ...
    const state::CharacterState& refreshedCharacter = refreshed.characters[refreshedCharacterIndex];

    std::vector<RandomizerItem> targets = collect_randomizer_items(refreshedCharacter, targetMode);
    ...
    for (const RandomizerItem& target : targets) {
        ...
        const RandomizerReplacementPool& replacementPool =
            ...
            randomizer_replacement_pool(
                target, replacementPools, target.weapon ? std::nullopt : armorClass);
        ...
        for (std::uint8_t lane = 0; lane < activeDetail.ordinarySocketCount; ++lane) {
            ...
            const std::span<const socket_plugs::Member> compatibleCandidates =
                ...
                compatible_randomizer_members(
                    socketCatalog, activeDefinition.definitionIndex, lane);
...
void randomize_character_inventory(const state::CharacterState& character) noexcept {
    ...
    const bool changed =
        result.insertedItems != 0 || result.replacedItems != 0 || result.randomizedSockets != 0;
...
[[nodiscard]] const char* randomizer_target_label(RandomizerTarget target) noexcept {
    ...
    case RandomizerTarget::weapons:
        return "Weapons";

    case RandomizerTarget::armor:
        return "Armor";
...
[[nodiscard]] const char* randomizer_perk_mode_label(RandomizerPerkMode mode) noexcept {
    ...
    case RandomizerPerkMode::normal:
        return "Normal / Compatible";

    case RandomizerPerkMode::fullyRandom:
        return "Fully Random";
    ...
    if (ImGui::BeginChild("gear_randomizer",
        ...
        if (randomizer_wants_armor(g_randomizerTarget)) {
            ...
            if (g_randomizerArmorClass == RandomizerArmorClass::all) {
                ...
                ImGui::TextDisabled(
                    "Specific classes use class art/name signals; ambiguous armor is excluded.");
            }
        }
...
void draw_editor_header() noexcept {
    ...
    // navigation clean while keeping the freeze risk visible at the top of Gear Editor.
    constexpr char kReplaceWarning[] =
        "Do not inspect items while replacing - may freeze the game.";
    constexpr float kWarningTextScale = 0.82F;

    const float warningStartX = ImGui::GetCursorPosX();
    ImGui::PushFont(nullptr, ImGui::GetStyle().FontSizeBase * kWarningTextScale);

    const float warningWidth = ImGui::CalcTextSize(kReplaceWarning).x;
    const float warningX =
        warningStartX + (std::max)(ImGui::GetContentRegionAvail().x - warningWidth, 0.0F);

    ImGui::SetCursorPosX(warningX);
    ImGui::TextColored(ImVec4(0.95F, 0.72F, 0.18F, 1.0F), "%s", kReplaceWarning);
    ImGui::PopFont();
    ...
    if (draw_editor_page_tab("RANDOMIZER", randomizer, scaled(150.0F), tabHeight, scaled(-1.0F))) {
...
void draw() noexcept {
    ...
    if (g_selectedLane >= item->detail.ordinarySocketCount
        || !editable_lane(*item, g_selectedLane)) {
