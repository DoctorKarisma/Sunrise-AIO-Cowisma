...
[[nodiscard]] bool fill_roster(const layouts::Definition& layout,
    ...
    for (std::size_t index = 0; index < layout.rosterGroupCount; ++index) {
        ...
        // Positions stay within the filtered live roster, so overrides and the mission seed
        // still find the surviving group. Public-target links retain the slot but retire it.
